...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        alpha=0,
        return_logits=False,   
        return_eos_state=False,
        return_state_idx=None,    
    ):
        r"""
        encoder_hidden_states  (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`):
            Sequence of hidden-states at the output of the last layer of the encoder. Used in the cross-attention if
        ...
        if return_eos_state:
            if return_state_idx is not None:
                batch_idx = range(0,sequence_output.size(0))
                
                return sequence_output[batch_idx,return_state_idx], CausalLMOutputWithCrossAttentions(
                    loss=lm_loss,
                ...
                return sequence_output[:,-1,:], CausalLMOutputWithCrossAttentions(
                    loss=lm_loss,
                    logits=prediction_scores,
                    past_key_values=outputs.past_key_values,
                    hidden_states=outputs.hidden_states,
